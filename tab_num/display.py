...
def display_tab_num_content(file_path=None, df=None):
    ...
    # Checks if the dataframe is loaded
    if df is not None:
        ...
        # Displays a select box with the list of numeric columns found
        selected_col = st.selectbox("Select a Numeric Column", num_col.cols_list)

        # Once the user selects a column, computes all the information to be displayed
        if selected_col:
            num_col.set_data(selected_col)

            # Displays an expander with the summary table
            with st.expander("Summary", expanded=True):
                summary_df = num_col.get_summary()
                st.table(summary_df)

            # Displays a histogram chart of the selected numeric column
            with st.expander("Histogram", expanded=True):
                st.altair_chart(num_col.histogram, use_container_width=True)

            # Displays the most frequent values in the selected numeric column
            with st.expander("Most Frequent Values", expanded=True):
                st.write(num_col.frequent)

    else:
        # If no dataframe is loaded, displays an error message
